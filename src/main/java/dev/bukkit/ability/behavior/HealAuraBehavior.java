 ...
 * {@code hasSetPassive("HEAL_AURA")} with a per-{@link ActiveAbility}
 * TickEvent subscription. One behavior instance per holder wearing the set.
 */
public class HealAuraBehavior implements AbilityBehavior {
    private ActiveAbility ctx;
    private long lastHealMs = 0;
    public HealAuraBehavior(ActiveAbility ctx) { this.ctx = ctx; }
    ...
    private void onTick(TickEvent e) {
        ...
        if (now - lastHealMs < HealAuraUtils.HEAL_INTERVAL_MS) return;
        lastHealMs = now;
        HealAuraUtils.tick(ctx.getHolder());
        HealAuraUtils.renderRing(ctx.getHolder());
    }
}
