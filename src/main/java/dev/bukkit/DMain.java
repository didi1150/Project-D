...
public final class DMain extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if (gameSettings.getDungeonWorld() != null && Bukkit.getWorld(gameSettings.getDungeonWorld()) == null) {
            Bukkit.createWorld(new WorldCreator(gameSettings.getDungeonWorld()).generator(new BukkitStoneWorldGenerator()));
        ...
        // ==============================================[ States ]=============================================================
        GameStateController gameStateController = new GameStateController(new BukkitTaskScheduler(this));
        if (gameSettings.isSetupMode()) {
            gameStateController.addState(new SetupState(eventBusInterface));
        }
        gameStateController.addState(new PreLobbyState(gameSettings.getPreLobbySpawn(), gameSettings.getMinPlayers(), 15, eventBusInterface));
        gameStateController.addState(new SelectClassState(gameSettings.getHoleCenter(), gameSettings.getSelectionSpawn(), gameSettings.getSelectionLocations(), eventBusInterface, progressionService));
        gameStateController.addState(new SelectItemState(eventBusInterface));
        gameStateController.addState(new ClearState(gameSettings.getHoleCenter(), eventBusInterface, effectManagerInterface, entityManager, progressionService,messageSenderInterface, this));
        ...
        CommandManager.getInstance(itemsConfig, progressionService, gameStateController, gameSettingsLoader, eventBusInterface).registerCommands(this);
